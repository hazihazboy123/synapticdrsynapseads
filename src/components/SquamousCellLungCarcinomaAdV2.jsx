...
/**
 * SquamousCellLungCarcinomaAd - Version 2 with Hard Cut Memes
 *
 * This version uses MemeCutaway for TikTok-style hard cuts where:
 * - Memes TAKE OVER the screen (no overlay)
 * - Meme audio plays (main audio paused)
 * - Hard cut back to content
 */
export const SquamousCellLungCarcinomaAdV2 = () => {
  ...
  // Calculate which frames have meme cutaways
  const memeCutawayFrames = memePlacements.memes.map(meme => {
    const triggerFrame = Math.floor((meme.adjustedTimestamp) * fps);
    const endFrame = triggerFrame + meme.durationFrames;
    return { start: triggerFrame, end: endFrame };
  });

  // Check if current frame is during a meme cutaway
  const isInMemeCutaway = memeCutawayFrames.some(
    ({ start, end }) => frame >= start && frame < end
  );

  return (
    <AbsoluteFill style={{ backgroundColor: '#0a0a0a' }}>
      {/* Audio narration - MUTED during meme cutaways */}
      <Audio
        src={audioPath}
        playbackRate={PLAYBACK_RATE}
        volume={isInMemeCutaway ? 0 : 1}
      />

      {/* System sounds - only when not in cutaway */}
      {!isInMemeCutaway && (
        <>
          <Sequence from={0}>
      ...
      {/* Main content - HIDDEN during meme cutaways */}
      {!isInMemeCutaway && (
        <>
          {/* Brain mascot */}
          <BrainMascot
      ...
      {/* HARD CUT MEME CUTAWAYS */}
      {memePlacements.memes && memePlacements.memes.map((meme, index) => (
        <MemeCutaway
          key={index}
          memeId={meme.id}
          triggerTimestamp={meme.adjustedTimestamp}
          playbackRate={1.0}  // Memes play at normal speed
          customDuration={meme.durationFrames / fps}
        />
      ))}
    </AbsoluteFill>
  );
...
export default SquamousCellLungCarcinomaAdV2;
